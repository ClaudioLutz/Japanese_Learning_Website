...
# -*- coding: utf-8 -*-
"""
This script creates a comprehensive Japanese lesson titled \"Die grundlegenden Partikel: は (wa), が (ga), を (o) und に (ni)\" for German-speaking learners.
"""
import os
import sys
import json
...
# --- Configuration ---
LESSON_TITLE = "Die grundlegenden Partikel: は (wa), が (ga), を (o) und に (ni)"
LESSON_DIFFICULTY = "Anfänger"
LESSON_DESCRIPTION = "Lerne die vier wichtigsten japanischen Partikel kennen! Diese Lektion erklärt einfach und verständlich die Funktionen von は (wa), が (ga), を (o) und に (ni) und zeigt dir, wie du sie korrekt anwendest, um natürliche japanische Sätze zu bilden."

# Lesson content pages configuration (explanation pages only)
CONTENT_PAGES = [
    {
        "page_number": 2,
        "title": "Seite 1: Was sind Partikel? - 助詞とは？ (Joshi to wa?)",
        "keywords": "Partikel, 助詞 (joshi), Grammatik, Satzbau, Funktion, Grundlagen",
        "image_concept": "Ein freundlicher japanischer Lehrer (Sensei) im Manga-Stil steht vor einer leeren Tafel. Er hält einen Zeigestock und deutet auf bunte Bausteine auf einem Tisch vor ihm. Die Bausteine haben die abstrakten Symbole für は, が, を, に und verbinden größere, neutrale Blöcke, die Wörter wie 'Person', 'Apfel' oder 'Haus' repräsentieren. Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Erklärung, was Partikel (助詞, joshi) sind, warum sie im Japanischen unverzichtbar sind und wie sie die Rolle eines Wortes im Satz bestimmen. Ein einfacher Vergleich mit deutschen Fällen (z.B. Nominativ, Akkusativ) wird als Lernhilfe angeboten."
    },
    {
        "page_number": 3,
        "title": "Seite 2: Das Thema des Satzes: Die Partikel は (wa) - 文のテーマ：助詞の「は」 (Bun no teema: Joshi no 'wa')",
        "keywords": "は (wa), Thema, topic marker, Kontrast, Vorstellung, です (desu)",
        "image_concept": "Ein junges Mädchen im Anime-Stil stellt sich in einem japanischen Klassenzimmer vor. Sie deutet mit dem Daumen auf sich selbst und lächelt freundlich. Über ihrem Kopf schwebt eine große, leere Sprechblase, deren Rand auffällig dick und betont ist, um den 'Rahmen' des Themas zu symbolisieren. Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Einführung in die Partikel は (wa) als Themenmarkierer. Es wird erklärt, wie は das Hauptthema eines Gesprächs oder Satzes festlegt. Anwendungsbeispiele wie bei der Selbstvorstellung ('Ich bin Student') und zur Kontrastierung werden erläutert."
    },
    {
        "page_number": 4,
        "title": "Seite 3: Das Subjekt identifizieren: Die Partikel が (ga) - 主語の特定：助詞の「が」 (Shugo no tokutei: Joshi no 'ga')",
        "keywords": "が (ga), Subjekt, subject marker, neue Information, Frage, Antwort",
        "image_concept": "Ein Junge schaut überrascht aus einem Fenster, während draußen die Sonne scheint. Er zeigt mit dem Finger auf die Sonne. Ein visueller Fokus, wie ein unsichtbarer Scheinwerfer, hebt die Sonne am Himmel hervor, um zu betonen, dass sie die 'neue' oder 'spezifische' Information ist. Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Einführung in die Partikel が (ga) als Subjektmarkierer. Die Lektion erklärt, wie が das spezifische Subjekt einer Handlung oder eines Zustands identifiziert, oft als Antwort auf eine 'wer/was'-Frage oder um eine neue, wichtige Information hervorzuheben (z.B. 'Der Hund bellt!')."
    },
    {
        "page_number": 5,
        "title": "Seite 4: Der feine Unterschied: は (wa) vs. が (ga) - 微妙な違い：「は」対「が」 (Bimyō na chigai: 'wa' tai 'ga')",
        "keywords": "は vs が, Vergleich, Kontext, alte Information, neue Information, Nuance",
        "image_concept": "Eine zweigeteilte Szene (Split-Screen). Links: Ein großer Elefant ist klar zu sehen. Eine Person zeigt auf seinen Rüssel. Der Fokus liegt auf der Eigenschaft des Rüssels (Thema: Elefant). Rechts: Mehrere verschiedene Tiere (Giraffe, Löwe, Elefant) sind kleiner dargestellt. Eine Person zeigt gezielt auf den Elefanten, um ihn als das Tier mit dem langen Rüssel zu identifizieren. Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Ein direkter Vergleich von は und が. Es wird das Konzept von 'altbekannter Information' (Thema, markiert durch は) gegenüber 'neuer, spezifischer Information' (Subjekt, markiert durch が) erklärt. Gegenüberstellungen mit Beispielsätzen zeigen die feinen Bedeutungsunterschiede auf."
    },
    {
        "page_number": 6,
        "title": "Seite 5: Das direkte Objekt: Die Partikel を (o) - 直接目的語：助詞の「を」 (Chokusetsu mokutekigo: Joshi no 'o')",
        "keywords": "を (o), Objekt, object marker, Akkusativ, Verb, Handlung",
        "image_concept": "Eine Person sitzt an einem Esstisch und liest ein Buch. Eine Hand hält das Buch, während die andere Hand eine Tasse Tee zum Mund führt. Die Handlung konzentriert sich klar auf das Buch (Objekt des Lesens) und die Tasse (Objekt des Trinkens). Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Einführung in die Partikel を (o) zur Kennzeichnung des direkten Objekts einer Handlung. Es wird erklärt, dass を das 'Opfer' oder Ziel einer Aktion markiert und fast immer mit einem transitiven Verb (z.B. essen, lesen, sehen) verwendet wird."
    },
    {
        "page_number": 7,
        "title": "Seite 6: Ort, Zeit und Ziel: Die Partikel に (ni) - 場所、時間、目的地：助詞の「に」 (Basho, jikan, mokutekichi: Joshi no 'ni')",
        "keywords": "に (ni), Ort, Zeit, Ziel, Richtung, Dativ, Existenz",
        "image_concept": "Eine dreigeteilte Collage. Oben links: Eine Wanduhr zeigt deutlich auf 3 Uhr, ein Pfeil deutet auf die Uhrzeit. Mitte: Ein Mädchen mit Schulranzen geht auf ein japanisches Schulgebäude zu, ein Pfeil zeigt die Richtung zum Gebäude. Unten rechts: Eine Katze sitzt unter einem Stuhl, ein Pfeil zeigt auf ihre exakte Position. Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Erklärung der drei häufigsten Funktionen von に für Anfänger: 1. Zeitpunkt einer Handlung (um 7 Uhr), 2. Ziel/Richtung einer Bewegung (nach Tokio gehen), 3. Ort der Existenz (in einem Raum sein)."
    },
    {
        "page_number": 8,
        "title": "Seite 7: Alle zusammen im Satz - 全てを文に (Subete o bun ni)",
        "keywords": "Satzbau, Anwendungsbeispiele, Kombination, Alltagssprache, Praxis",
        "image_concept": "Eine Szene in einer Bibliothek. Eine junge Frau (Thema は) überreicht einem jungen Mann (Empfänger に) ein Buch (Objekt を). Die Handlung des Gebens verbindet alle Elemente. Beide Personen lächeln. Der Hintergrund ist eine japanische Bibliothek mit Bücherregalen. Niedlicher Manga/Anime-Stil mit klaren Linien und kultureller Authentizität. WICHTIG: Kein Text, keine Schrift, keine Schilder oder Zeichen sollten im Bild sichtbar sein.",
        "content_focus": "Zusammenführung aller vier Partikel in komplexeren Beispielsätzen. Analyse von Sätzen, die zeigen, wie die Partikel zusammenarbeiten, um eine vollständige Aussage zu bilden, z.B. 'Frau Tanaka gibt Herrn Sato ein Buch.' (Tanaka-san wa Satō-san ni hon o agemasu)."
    }
]

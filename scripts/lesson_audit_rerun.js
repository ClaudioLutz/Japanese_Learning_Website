export const meta = {
  name: 'lektions-didaktik-audit-rerun',
  description: 'Gedrosselter Nachlauf der rate-limited fehlgeschlagenen Audit-Gruppen (Chunks statt Burst)',
  phases: [
    { title: 'Audit', detail: 'fehlgeschlagene Gruppen in kleinen Chunks nachholen' },
...
const FINDINGS_SCHEMA = {
  ...
  properties: { findings: { type: 'array', items: {
    ...
    properties: {
      ...
      fundstelle: { type: 'string' }, problem: { type: 'string' },
      vorschlag: { type: 'string' }, beleg: { type: 'string' },
    },
  } } },
}
const VERDICT_SCHEMA = {
  type: 'object', required: ['ist_echt', 'begruendung', 'vorschlag_korrekt'], additionalProperties: false,
  properties: {
    ist_echt: { type: 'boolean' }, begruendung: { type: 'string' },
    vorschlag_korrekt: { type: 'boolean' },
...
- faktischer_fehler: falsche Lesung (reading/Furigana/Romaji), falsche deutsche Übersetzung (meaning_de), falsche Beispielsatz-Übersetzung, falsche Onyomi/Kunyomi, falsche Kanji-Bedeutung.
...
2. Ist es ein KLARER, verteidigbarer didaktischer Fehler? Bei faktischen/Quiz-Findungen: leite die korrekte Lesung/Übersetzung/Antwort selbst her und vergleiche.
...
const auditStage = (g) => agent(auditPrompt(g), { label: `audit:${g.group_id}`, phase: 'Audit', schema: FINDINGS_SCHEMA })
...
phase('Audit')
log(`Gedrosselter Nachlauf: ${GROUPS.length} Gruppen in Chunks à ${CHUNK}`)
const perGroup = []
for (let i = 0; i < GROUPS.length; i += CHUNK) {
  ...
  const part = await pipeline(chunk, auditStage, verifyStage)
  perGroup.push(...part)
}

const all = perGroup.flat().filter(Boolean)
const confirmed = all.filter((f) => f.verdict && f.verdict.ist_echt)
log(`Nachlauf fertig — Roh-Findungen: ${all.length} | bestätigt: ${confirmed.length}`)
